...
_model_sample_model = None


def load_model():

    model_path = os.path.join(os.path.dirname(__file__), "1.pt")
    ...
    global _model_sample_model
    if _model_sample_model is None:
        try:
            _model_sample_model = YOLO(model_path)
            print("Модель успешно загружена!")
        except Exception as e:
            print(f"Ошибка при загрузке модели: {e}")
            raise e
    return _model_sample_model
...
def get_image_from_path(image_path: str) -> Image:
    """Load image from path and convert to PIL RGB format
...
def add_bboxs_on_img(image: Image, predict: pd.DataFrame()) -> Image:
    ...
    annotator = Annotator(np.array(image), pil=True, font_size=40)
